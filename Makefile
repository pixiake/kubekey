...
.PHONY: kk
kk:
	CGO_LDFLAGS="-Wl,-z,relro,-z,now,-z,noexecstack" CGO_ENABLED=1 CC=aarch64-linux-gnu-gcc go build -trimpath -tags "$(BUILDTAGS)" -ldflags "$(LDFLAGS) -s -w -linkmode=external" -trimpath -buildmode=pie -o $(BIN_DIR)/kk github.com/kubesphere/kubekey/v3/cmd/kk;
